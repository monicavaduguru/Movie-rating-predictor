...
def recommender2(user_train,user_test,movie_train,k):
    sim_user={}
    sim_userabs={}
    ...
    for user in user_test:
        ...
        for user1 in user_train:
            if user!=user1:
                ...
                sim=pearson_sim_caseamp(user_train[user1],user_test[user],IUF_train)
                dict1[user1]=sim
                dict2[user1]=abs(sim)
        sim_user[user]=dict1
    ...
    #print sim_user[201]
    #print sorted(sim_user[3].items(), key=itemgetter(1), reverse=True)
    ...
    for user in user_test:
        ...
        for movie in user_test[user]:
            if user_test[user][movie]==0:
                ...
                rating=reco_pearson(user,movie,user_train,user_test,topklist,sim_user,user_avg,k)
                #print rating
